...
void main() {
  ...
  group('Server-initiated request dispatch', () {
    ...
    test('sampling/createMessage routes to onSamplingRequest handler',
        () async {
      var called = 0;
      ...
      final response = transport.sentMessages
          .firstWhere((m) => m['id'] == 'srv-7', orElse: () => {});
      expect(response['result']?['model'], equals('m'));
    });

    test('roots/list returns locally registered roots by default',
        () async {
      ...
      await Future<void>.delayed(const Duration(milliseconds: 10));
      final response = transport.sentMessages
          .firstWhere((m) => m['id'] == 'srv-8', orElse: () => {});
      final roots = response['result']?['roots'] as List?;
      expect(roots, hasLength(1));
      expect(roots?.first['uri'], equals('file:///workspace'));
    });

    test('elicitation/create routes to onElicitationRequest handler',
        () async {
      ...
      final response = transport.sentMessages
          .firstWhere((m) => m['id'] == 'srv-9', orElse: () => {});
      expect(response['result']?['action'], equals('accept'));
    });

    test('unknown server-initiated method returns method-not-found error',
        () async {
      await client.connect(transport);
      transport.simulateMessage({
        'jsonrpc': McpProtocol.jsonRpcVersion,
      ...
      await Future<void>.delayed(const Duration(milliseconds: 10));
      final response = transport.sentMessages
          .firstWhere((m) => m['id'] == 'srv-10', orElse: () => {});
      expect(response['error']?['code'], equals(McpProtocol.errorMethodNotFound));
    });
  });
